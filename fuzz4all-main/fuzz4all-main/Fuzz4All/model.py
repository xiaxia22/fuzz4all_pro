...
def make_model(eos: list, model_name: str, device: str, max_length: int):
    if is_ollama_model(model_name):
        ...
        return ChatStyleModel(model_name, device, eos, max_length)
...
class ChatStyleModel:
    def __init__(
        self, model_name: str, device: str, eos: List[str], max_length: int
    ) -> None:
        self.device = device
    ...
    def generate(
        ...
        gen_seqs = raw_outputs[:, len(input_tokens[0]) :]
        gen_strs = self.tokenizer.batch_decode(
            gen_seqs, skip_special_tokens=self.skip_special_tokens
        )
        outputs = []
        for output in gen_strs:
            ...
            clean_output = parsed if parsed else output
            min_index = 10000
            for eos in self.eos:
                if eos in clean_output:
                    min_index = min(min_index, clean_output.index(eos))
            outputs.append(clean_output[:min_index])
        return outputs
