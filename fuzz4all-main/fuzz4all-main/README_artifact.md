...
- If your gpu has <32 GB, you can switch to a smaller model with a smaller batch size (for example a smaller Qwen2.5-Coder checkpoint and `export FUZZING_BATCH_SIZE=5`)
- If there are still out of memory errors, unfortunately you may need to switch to CPU mode and use smaller models.

> This repository is now configured by default for `Qwen/Qwen2.5-Coder-7B-Instruct` and can also use local Ollama Qwen2.5 models.

Before you start, you need to export your DeepSeek API key to the environment variable `DEEPSEEK_API_KEY`:
```
...
Fuzzing 鈥? 30% 鈹佲攣鈹佲攣鈹佲攣鈹佲攣鈹佲攣鈹佲攣鈺衡攣鈹佲攣鈹佲攣鈹佲攣鈹佲攣鈹佲攣鈹佲攣鈹佲攣鈹佲攣鈹佲攣鈹佲攣鈹佲攣鈹佲攣鈹佲攣  30/100 鈥?0:02:56
```
...
>We recommend first running with a smaller model (i.e., `export FUZZING_MODEL="Qwen/Qwen2.5-Coder-3B-Instruct`) and a smaller batch size (e.g., `export FUZZING_BATCH_SIZE=5`)
...
18158 4729
18573 4786
Fuzzing 鈥? 29% 鈹佲攣鈹佲攣鈹佲攣鈹佲攣鈹佲攣鈹佲暫鈹佲攣鈹佲攣鈹佲攣鈹佲攣鈹佲攣鈹佲攣鈹佲攣鈹佲攣鈹佲攣鈹佲攣鈹佲攣鈹佲攣鈹佲攣鈹佲攣  29/101 鈥?0:00:46
```
