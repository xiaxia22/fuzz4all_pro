...
**Source:** `java.management\javax\management\MXBean.html`
...
(though it can be). On
the other hand, we have added a
...
the MXBean framework

converts

model-specific classes into
...
long used = (Long) usage.
get
("used");
```

For attributes with simple types like
...
. This is the recommended way to interact with
...
. In the example above, the MXBean
interface is
...
method in an MXBean interface must be
...
is ignored when looking for getters.
- A method
...
specifies that
there is a readable attribute called
...
for the same name, then the type
...
, the MXBean mapping is described
by the following information:

- The corresponding Open Type,
...
, which is
always the same for any given

opentype(J)

. This is a Java
class.
- How a value is converted from type
...
If no mapping rules exist to derive
...
cannot be the type of a method
...
. All method parameters in an MXBean
...
(an array with non-primitive element type
...
corresponding boxed types from
...
An array of primitive type such as

long[]

can be represented
directly as an Open Type. Thus,
...
In practice, the difference between a plain
...
is a class or interface that implements
...
interface, can reference another MXBean interface,
...
If you make an MXBean proxy for a
...
, another MXBean interface, it does so with
...
; or it can have been created using
...
Given a Java class or interface
...
any getter has a type that is not convertible, then
...
defines an attribute called

Owner

, while
in a Java Bean or mapped
...
defines a property or item called
...
. The mapping from an instance of
...
just described is done as follows. First, if
...
will have been mapped to an item with name "
...
is an interface. The concrete class that is
used to represent a type as Open Data is
...
Reconstructing an instance of Java type
...
is not
reconstructible. If any item in the

CompositeData

is not
reconstructible, then

J

is not reconstructible either.

For any given

J

, the following rules are consulted to
determine how to reconstruct instances of
...
has at least one public
constructor with either
...
annotations (these may correspond to
...
where not all the items were present. A
constructor is

applicable

if all the properties named
in its
...
annotation are present as items in the
...
has a public no-arg constructor, and
for every getter in
...
there is a corresponding setter
with the same name and type, then an instance of

J

is
constructed with the no-arg constructor and the setters are
called with the reconstructed items from the
...
, some items might not be present. In this case,
the corresponding setters will not be called.
- Otherwise, if

J

is an interface that has no methods
other than getters, an instance of
...
targeting a runtime that does not include the
...
is compiled with a public constructor
and the
...
interfaces. This is a consequence of the immutability of
...
operation return value is a primitive type such as
...
rules and do not have a corresponding
...
interface.
Otherwise, it will be the result of calling
...
specified by the mapping rules above. So even when
...
Sometimes the unmapped type

T

of a method parameter or
return value in an MXBean must be represented as a string. If

T

is a non-generic type, this string is the value
returned by
...
is a non-generic non-array type,
...
is a parameterized type such as
...
is the first
type parameter; if there is a second type parameter
...
Open types is signaled with an

OpenDataException

. This
can happen when an MXBean interface is being analyzed, for
example if it references a type like
...
according to the rules above, in a
...
boolean True if the annotated interface is an MXBean interface.
...
**Returns:** true if the annotated interface is an MXBean interface.
