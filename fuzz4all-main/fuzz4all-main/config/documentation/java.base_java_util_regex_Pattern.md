...
**Source:** `java.base\java\util\regex\Pattern.html`
...
The character with Unicode character name
...
are supported by the named character construct
...
are supported by the grapheme
cluster matcher

\X

and the corresponding boundary matcher
...
is specified and the number of combining
marks for any character is too large, an
...
is deserialized, compilation is deferred
until a direct or indirect invocation of this method. Thus, if a
deserialized pattern has

CANON_EQ

among its flags and the number
...
**Since:** 1.4
**See Also:** String.split(String, int)
...
COMMENTS
**Returns:** the given regular expression compiled into a pattern with the given flags
...
**API Note:** This method creates a predicate that behaves as if it creates a matcher
from the input sequence and then calls
