...
**Source:** `java.sql.rowset\javax\sql\rowset\CachedRowSet.html`
...
supports getting data only from a
...
An application can modify the data in a
...
while its data is being modified. Being disconnected makes a

RowSet

object much leaner and therefore much easier to pass to another component. For
example, a disconnected
...
The following line of code creates the
...
that is initialized with default values except that its

SyncProvider

object is the one specified.
...
The following line of code resets the
...
object by using the
getter methods inherited from the

ResultSet

interface. The following examples, in which
...
An application can get information about the columns in a
...
. The second line finds out how many columns
...
object must also call the method
...
preceding two code fragments call the method
...
Object."
The default reference implementation provider,
...
both being the values with which the

RowSet

object was initially
populated. However, once any values in the
...
bar graph classes must implement the

RowSetListener

interface.
Then they can be added to the
...
A second major use for

CachedRowSet

objects is to provide
scrolling and updating for
...
in the following code fragment, where
...
does.
The difference is that the cursor for
...
can move only forward. In addition,
...
similar to the setter methods provided by the
...
methods for setting the elements in its
...
, it will in turn call on this

RowSet

object's reader,
which will in turn invoke its
...
the command. Simply by calling the
...
object can retrieve data from a

ResultSet

object in chunks of data, called
...
.
The following lines of code demonstrate using the method

populate

.
Note that this version of the method takes two parameters, a

ResultSet

handle and the row in the
...
will be populated with four rows from

rsHandle

starting with the tenth row.

The next code fragment shows populating a
...
, which may or may not take a

Connection

object as a parameter. This code passes
...
fragment and the previous one. First, the method

setMaxRows

is not
called, so there is no limit set for the number of rows that
...
always has the overriding limit of how much data it can
store in memory.) The second difference is that the you cannot pass the method

execute

the number of the row in the
...
will contain five rows of data from the

ResultSet

object produced by the command for
...
to connect to the data source and
execute the command for

crs

. An application is then able to operate on the
data in

crs

in the same way that it would operate on data in any other
...
with 1000 rows of data. If the page size has been set to 100, the first
call to the method
...
object
containing the first 100 rows of

rs

. After doing what it needs to do with the
...
object. After the tenth call to the method
...
, which are stored in memory. At any given time, the data from only one
...
page is not the last page of rows and
...
interface also defines the method
...
as is done in the following code fragment, the method
...
meaning that the cursor for the tenth
...
After going through all of the rows in the last page (the
...
to commit the changes when

acceptChanges()

is called. If
set to false, the changes will

not

be committed until one of the

CachedRowSet

interface transaction methods is called.
...
This method can be used as an alternative to the

execute

method when an
application has a connection to an open
...
object is disconnected except when
...
object. This method also closes

conn

after it has populated this
...
object (writer). The writer will attempt
to propagate changes made in this
...
implementation, for example, throws a

SyncProviderException

and does not write anything to the
data source.

An application may choose to catch the
...
object lists the conflicts row by row and
...
method again to write resolved values to the
data source. If all of the values in the data source are already the
values to be persisted, the method
...
writing changes to the data source when the method
...
method will determine if the

COMMIT_ON_ACCEPT_CHANGES

is set to true or not. If it is set
to true, all updates in the synchronization are committed to the data
source. Otherwise, the application
...
object already defined within the
...
properties must be used by the
...
the original value is the value with which this
...
object should lock until its contents and
associated updates are fully cleared, thus preventing 'dirty' reads by
other components that hold a reference to this

RowSet

object.
In addition, the contents cannot be released
until all components reading this

CachedRowSet

object
have completed their reads. This
...
property be
relevant to the originating data source from which this

CachedRowSet

object was originally established.

This method empties a rowset, as opposed to the
...
- if (1) the current row has not been deleted or
...
may be called at any time during the lifetime of a
...
- if the cursor is before the first row or after the last
row in this
...
identifying the column to be checked for updates

**Returns:**
- true

if the designated column has been visibly updated;
...
- if the cursor is on the insert row, before the first row,
...
if the column has been visibly updated;
...
implementation or a specialized
...
column value must be represented as a
...
object for the rowset, with the
values in each row being contained in
...
type of collection guarantees that the map will be in
ascending key order, sorted according to the natural order for the
key's class.
Each key references a
...
object. Therefore, the size of each

Vector

object must be exactly equal to the number of
columns in the
...
available within the internal

RowSet

tabular structure by
virtue of a key already set either on the
...
object that contains the values in
each row in this
...
object. Implementations have some latitude in
how they can represent this
...
that most implementations will do the same. If a
...
- if an error occurs generating the collection or
...
to allow the rowset to be populated.
...
is a private, accessible field within
the rowset implementation, when an application calls the

execute

method, it in turn calls on the reader's
...
object, including information about the
...
object that was set when the rowset
...
implementation should always be instantiated
with an available

SyncProvider

mechanism, but there are
cases where resetting the
...
object for a time and then choose to use a provider
...
. This has the effect of resetting
...
object is reading the contents of a rowset,
it creates a

RowSetMetaData

object and initializes
it using the methods in the
...
this method is called internally to pass the
...
object containing
metadata about the columns in this
...
- if invalid metadata is supplied to the
rowset
...
object is the value it had before
...
object was populated. This method is called internally
when an application calls the method
...
object has been implemented to check for conflicts.
...
object that contains the original value for
this
...
object. This name may be set on multiple occasions,
...
object giving the name of the table that is the
...
object's values during a synchronization attempt.
...
object may obtain the
the name internally from the
...
object identifying the table from which this

CachedRowSet

object was derived; cannot be
...
indicating the columns that form
...
- if any of the numbers in the given array
...
object. In effect, both

CachedRowSet

objects have a cursor over the same data.
...
NOTE: If multiple threads access
...
method, the following behavior is specified
to preserve shared data integrity: reads and writes of all
shared

RowSet

objects should be made serially between each
...
object and that has a cursor over
the same data

**Throws:**
- SQLException

- if an error occurs or cloning is not
...
call, updates made to the copy of the original

RowSet

object
must not be visible to the original
...
must not have scope over the new

RowSet

copies. In addition, any constraint restrictions
...
- if an error occurs in generating the copy of
the of this
...
object. In addition, primary
or foreign key constraints set in the originating

CachedRowSet

object must
be equally enforced in the new empty
...
method
call, updates made to a copy of this
...
method call, updates made to a copy of this
...
object must not have scope over the new

RowSet

object. In addition, any constraint restrictions
...
Makes all changes that are performed by the
...
transaction marker. This method should be used only
...
object has populated a number of additional rows. The

numRows

parameter
ensures that this event will only be fired every
...
object that is the source of the events
- numRows
...
from where to start
populating the records in this
...
fetches an additional page according to the
...
- if an error occurs fetching the next page, or if this
...
object
**See Also:** execute(java.sql.Connection)
...
or an error occurs in establishing the connection to the
data source
**See Also:** populate(java.sql.ResultSet)
...
synchronization provider's writer fails to write the updates
back to the data source
**See Also:** acceptChanges(java.sql.Connection)
...
object to its previous value
**See Also:** RowSetListener.rowSetChanged(javax.sql.RowSetEvent)
...
object
**See Also:** setSyncProvider(java.lang.String)
...
null
**See Also:** RowSetMetaData.setTableName(int, java.lang.String)
...
**Returns:** RowSetWarning the first
