...
**Source:** `java.sql\java\sql\Connection.html`
...
configuration when there is a JDBC method available. By default a
...
A user may create a new type map, which is a

java.util.Map

object, make an entry in it, and pass it to the
...
The code fragment retrieves the type map for the
...
affect which methods throw certain
...
that an application explicitly
...
object has been
closed. A connection is closed if the method
...
prepare operation takes place immediately when the
...
If this method is called during a transaction, the result
...
Subsequent warnings will be chained to this
SQLWarning.
...
You must set the values for the
...
object to the given
holdability. The default holdability of
...
The JDBC driver is responsible for mapping the elements

Object

array to the default JDBC SQL type defined in
java.sql.Types for the given class of
...
the objects, with the exception of the
...
: This method is intended to address a rare but serious
...
When the driver determines that the
...
required that implementations throw a
...
The pooling manager does not need to call
...
objects
- Returns a logical connection handle when
...
is called without an initial call to
...
#### default boolean setShardingKeyIfValid​(
ShardingKey
shardingKey,
...
Sets and validates the sharding keys for this connection. A
...
sharding key is vendor-specific. Consult your vendor's
documentation for additional information.

**Parameters:**
- shardingKey

- the sharding key to be validated against this connection.
The sharding key may be

null
- superShardingKey

- the super sharding key to be validated against this
...
Sets and validates the sharding key for this connection. A
...
- the sharding key to set on this connection. The sharding
key may be
...
- if an error occurs setting the sharding key; or
this method is called on a closed
...
**Since:** 1.1
**See Also:** DriverManager.getConnection(java.lang.String, java.util.Properties)
...
- if a database access error occurs
or this method is called on a closed connection
**See Also:** setAutoCommit(boolean)
...
**Returns:** the current catalog name or
...
- if a database access error occurs
or this method is called on a closed connection
**See Also:** setCatalog(java.lang.String)
...
constants
**See Also:** DatabaseMetaData.supportsTransactionIsolationLevel(int)
...
is less than 0
**Since:** 1.6
**See Also:** DatabaseMetaData.getClientInfoProperties()
...
**Returns:** the current schema name or
...
or this method is called on a closed connection
**Since:** 1.7
**See Also:** setSchema(java.lang.String)
...
this method
**Since:** 1.7
**See Also:** setNetworkTimeout(java.util.concurrent.Executor, int)
...
**API Note:** This method is to be used by Connection pooling managers.
...
to the cache.
**Implementation Requirements:** The default implementation is a no-op.
**Throws:** SQLException
...
**API Note:** This method validates that the sharding keys are valid for the
...
be completed, otherwise the validation process is aborted. A value of 0 indicates
the validation process will not time out.
**Returns:** true if the connection is valid and the sharding keys are valid
and set on this connection; false if the sharding keys are not valid or
the timeout period expires before the operation completes.
**Throws:** SQLException
...
**API Note:** This method validates that the sharding key is valid for the
...
be completed,else the validation process is aborted. A value of 0 indicates
the validation process will not time out.
**Returns:** true if the connection is valid and the sharding key is valid to be
set on this connection; false if the sharding key is not valid or
the timeout period expires before the operation completes.
**Throws:** SQLException
...
**API Note:** This method sets the specified sharding keys but does not require a
round trip to the database to validate that the sharding keys are valid
for the
...
**API Note:** This method sets the specified sharding key but does not require a
round trip to the database to validate that the sharding key is valid
for the
