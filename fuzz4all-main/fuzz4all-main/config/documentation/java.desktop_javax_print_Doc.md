...
**Source:** `java.desktop\javax\print\Doc.html`
...
, and the Print Job calls methods
on that object to obtain the print data. The
...
), in which the print data is available. A doc
...
. The doc returns its printing attributes
stored in an
...
Once the Print Job has called

getPrintData()

and
obtained the stream, any further calls to

getPrintData()

will return the same stream object upon which reading may
already be in progress,

not

a new stream object that will re-read the
...
affords considerable implementation flexibility. The
...
methods, or because the print data source is already an
...
, then the print service should always close these streams
...
stream does not mean that it will, with the implications that
...
**All Known Implementing Classes:** SimpleDoc
...
**Returns:** reader for reading the print data characters from this doc. If a
reader cannot be provided because this doc does not meet the
criteria stated above,
