...
const phase2 = [
  ...
  { icon: '⚙️', title: '编译与验证', desc: 'javac 编译 + 运行验证，区分 SAFE / FAILURE / ERROR / TIMEOUT' },
  ...
  <div class="page-inner">
    <div class="hero-band">
      ...
      <p>
        ...
        seed 生成、分类感知 mutation、编译运行验证以及反馈闭环。当前展示页统一采用 10 类代表 API 的 200 样本口径结果，
        重点体现不同类别在分类驱动与反馈收敛之后的稳定效果，而不是只展示单次历史峰值。
      </p>
      <div class="hero-meta">
        <span>陕西科技大学</span>
        <span>夏文静</span>
    ...
    <div class="g4" style="margin-bottom:20px">
      ...
      <StatCard :value="2000" label="总生成测试用例" />
      <StatCard :value="'1205'" label="编译通过 (SAFE)" color="var(--green)" />
      <StatCard :value="'38+'" label="领域变异算子" />
    </div>

    <div class="card" style="margin-bottom:16px">
      <div class="card-title">系统工作流程</div>
      <div style="font-size:11px;color:var(--muted);margin-bottom:12px;padding:4px 10px;background:var(--surface2);border-radius:5px;border-left:3px solid var(--light);display:inline-block">
        阶段一：离线构建分类知识库
      </div>
      <WorkflowDiagram :steps="phase1" style="margin-bottom:20px" />
      <div style="font-size:11px;color:var(--primary);margin-bottom:12px;padding:4px 10px;background:var(--primary-xl);border-radius:5px;border-left:3px solid var(--primary);display:inline-block">
        阶段二：在线自适应闭环生成
      </div>
