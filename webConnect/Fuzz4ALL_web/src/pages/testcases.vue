...
import { deleteCode, getCodes, uploadCode, type CodeRecord } from '../services/backendApi'

const loading = ref(false)
const error = ref('')
const rows = ref<CodeRecord[]>([])
const selectedRows = ref<CodeRecord[]>([])
...
async function loadData() {
  ...
  error.value = ''
  try {
    rows.value = await getCodes()
  } catch (e) {
    const message = e instanceof Error ? e.message : '请求失败'
    error.value = `加载测试用例列表失败：${message}`
    rows.value = []
  } finally {
    loading.value = false
  }
...
function onSelectionChange(selection: CodeRecord[]) {
...
async function deleteSelected() {
  ...
  await Promise.all(selectedRows.value.map((item) => deleteCode(item.id)))
